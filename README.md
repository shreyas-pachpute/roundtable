...
> **Status: v0 runs.** **Try it at [roundtable.shreyaspachpute.in](https://roundtable.shreyaspachpute.in)**, no key needed. The Harbor Logistics meeting runs end to end: all nine seats, fan-out estimation with a flagged disagreement, price and risk in parallel, six drafted sections, objections with evidence, a defence, an Arbiter's ruling, two clauses escalated to your seat, and the five deliverables. Bring your own key (Anthropic, or any OpenAI-compatible endpoint such as vLLM) from the settings panel, or run the keyless mock to see the mechanics. See [Running it](#running-it). What is still a plan is in [`docs/PLAN.md`](docs/PLAN.md) and the [issues](../../issues).
